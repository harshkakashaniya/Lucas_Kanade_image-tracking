...
def Pipeline():
    ...
    for i in range(start,end):
        ...
        if count==0:
            ...
            #template=cv2.cvtColor(image.copy(),cv2.COLOR_BGR2GRAY)
        ...
        rect_img = cv2.polylines(image,  np.int32([new_vtx.T]),  1,  (0, 0, 200),  2)
        #cv2.imshow('rect',rect_img)
        print(count)
        count += 1
        print('Frame processing index')
        print(i)
        cv2.imwrite('%d.jpg' %count,rect_img)
        img_array.append(rect_img)
        success, image = vidObj.read()

    return img_array,size

def video(img_array,size):
    video=cv2.VideoWriter('car1.avi',cv2.VideoWriter_fourcc(*'DIVX'), 16.0,size)
